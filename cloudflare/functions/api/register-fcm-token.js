...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!player) return new Response(JSON.stringify({ error: 'not_registered' }), { status: 403 });
    if (player.fcmToken === token) return new Response(JSON.stringify({ ok: true, skipped: true }), { headers: { 'Content-Type': 'application/json' } });
