...
/**
 * Stores the answer once, creates/removes its reminder, and notifies the
 * person who created the appointment on both web push and Android FCM.
 */
export async function onRequestPost(context) {
  ...
  try {
    ...
    // One write is important: KV has a per-key write window on the free plan.
    await env.DEUCE_KV.put(`invites:${inviteId}`, JSON.stringify(data));

    if (accepted && data.toEmail && data.dateTime) {
    ...
    if (fromKey) {
      ...
      if (sender?.subscription && env.VAPID_PUBLIC_KEY && env.VAPID_PRIVATE_KEY) {
        try {
          ...
          console.error('Invite response web push error:', error);
        }
      }
    }
